...
class ExecutiveSummaryAgent:
    ...
    def run(
        ...
        # Validate that the input risk_results and negotiation_results are dictionaries
        ...
        # Validate that both inputs carry the keys this agent actually reads
        if "risks" not in risk_results:
                raise ValueError("risk_results must contain a 'risks' key.")

        if "negotiation_items" not in negotiation_results:
                raise ValueError("negotiation_results must contain a 'negotiation_items' key.")
